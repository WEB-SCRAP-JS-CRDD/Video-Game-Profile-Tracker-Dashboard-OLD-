...
class _ChampionsPageState extends State<ChampionsPage> {
  ...
  double _opacity = 1.0;
  Timer? _timer;
  final double iconSize = 32.0; // Increase icon size as needed
  final TextStyle textStyle =
      TextStyle(fontSize: 22); // Increase font size as needed
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      filteredChampions
          .sort((a, b) => compareString(ascending, a.name, b.name));
    } else if (columnIndex == 1) {
      filteredChampions
          .sort((a, b) => compareString(ascending, a.winRate, b.winRate));
    } else if (columnIndex == 2) {
      filteredChampions
          .sort((a, b) => compareString(ascending, a.banRate, b.banRate));
    } else if (columnIndex == 3) {
      filteredChampions
          .sort((a, b) => compareNumber(ascending, a.matches, b.matches));
    }
    setState(() {
                    ...
                    children: [
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              // List of icon buttons for each role
                              for (var role in [
                                'ALL',
                                'ADC',
                                'SUPP',
                        ...
                        Expanded(
                          child: SingleChildScrollView(
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Container(
                                decoration: BoxDecoration(
                                  color: AppColors.back.withOpacity(
                                      0.9), // Ici, '0.7' est l'opacité du fond blanc. Vous pouvez ajuster cette valeur entre 0.0 et 1.0.
                                  borderRadius: BorderRadius.circular(
                                      20), // Optionnel: pour ajouter des bords arrondis au tableau
                                ),
                                child: DataTable(
                                  sortAscending: isAscending,
                                  sortColumnIndex: sortColumnIndex,
                                  columnSpacing:
                                      200, // Ajustez l'espacement si nécessaire
                                  horizontalMargin:
                                      10, // Ajustez les marges si nécessaire
                                  columns: <DataColumn>[
                                    DataColumn(
                                      label: Text('Champion',
                                          style: TextStyle(
                                    ...
                                    DataColumn(
                                      label: Text('Rank Icon',
                                          style: TextStyle(
                                              fontWeight: FontWeight.bold)),
                                    ),
                                  ],
                                  rows: filteredChampions
